...
public:
    ...
    PointData() = default;
    inline PointData(uint32_t n_points, uint32_t n_total_points, uint32_t n_dim) : coords(n_points * n_dim, 0),
                                                                                   cluster_ids(NONE_CLUSTER),
                                                                                   n_total_points(n_total_points),
                                                                                   n_dim(n_dim) {}

    inline float* getPointCoords(uint32_t idx) { return &coords[idx * n_dim]; }
    inline int* getPointClusterID(uint32_t idx) { return &cluster_ids[idx * n_dim]; }
    ...
    {
        return PointRef{&coords[idx * n_dim], &cluster_ids[idx * n_dim], idx, n_dim};
    }
};